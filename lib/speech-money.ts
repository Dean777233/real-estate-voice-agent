...
/** Convert a non-negative integer up to 999,999,999 into English words. */
export function integerToWords(n: number): string {
  if (!Number.isFinite(n) || n < 0) {
    throw new RangeError("integerToWords expects a non-negative finite number");
  }
  const amount = Math.round(n);
  if (amount === 0) return "zero";
  if (amount >= 1_000_000_000) {
    throw new RangeError("integerToWords supports amounts under one billion");
  }
...
/** e.g. 285000 → "two hundred eighty-five thousand dollars" */
export function formatDollarsForSpeech(amount: number): string {
  const dollars = normalizeDollarAmount(amount);
  const label = dollars === 1 ? "dollar" : "dollars";
  return `${integerToWords(dollars)} ${label}`;
}

/** e.g. 2200 → "twenty-two hundred dollars per month" */
export function formatDollarsPerMonthForSpeech(amount: number): string {
  const dollars = normalizeDollarAmount(amount);
...
/** e.g. 26400 → "twenty-six thousand four hundred dollars per year" */
export function formatDollarsPerYearForSpeech(amount: number): string {
